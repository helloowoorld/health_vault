...
-- Seed dummy data with fixed IDs but without foreign key constraints
DO $$
DECLARE
...
BEGIN
  ...
  IF profile_count = 0 THEN
    -- Create UUIDs for our entities
    DECLARE
      patient1_id uuid := '00000000-0000-0000-0000-000000000001'::uuid;
      doctor1_id uuid := '00000000-0000-0000-0000-000000000002'::uuid;
      pharma1_id uuid := '00000000-0000-0000-0000-000000000003'::uuid;
      patient2_id uuid := '00000000-0000-0000-0000-000000000004'::uuid;
      patient3_id uuid := '00000000-0000-0000-0000-000000000005'::uuid;
      doctor2_id uuid := '00000000-0000-0000-0000-000000000006'::uuid;
    BEGIN
      ...
      VALUES
        ('00000000-0000-0000-0000-000000000101'::uuid, patient1_id, doctor1_id, now() + interval '2 days', 'pending', now()),
        ('00000000-0000-0000-0000-000000000102'::uuid, patient1_id, doctor1_id, now() + interval '5 days', 'confirmed', now()),
        ('00000000-0000-0000-0000-000000000103'::uuid, patient1_id, doctor1_id, now() - interval '3 days', 'completed', now()),
        ('00000000-0000-0000-0000-000000000104'::uuid, patient2_id, doctor2_id, now() + interval '1 day', 'pending', now()),
        ('00000000-0000-0000-0000-000000000105'::uuid, patient3_id, doctor2_id, now() + interval '3 days', 'confirmed', now()),
        ('00000000-0000-0000-0000-000000000106'::uuid, patient2_id, doctor2_id, now() - interval '5 days', 'completed', now());

      -- Insert dummy documents
      INSERT INTO documents (id, user_id, name, type, ipfs_hash, test_date, created_at)
      VALUES
        ('00000000-0000-0000-0000-000000000201'::uuid, patient1_id, 'Blood Test Results', 'medical_report', 'ipfs_hash_123', now() - interval '12 days', now() - interval '10 days'),
        ('00000000-0000-0000-0000-000000000202'::uuid, patient1_id, 'X-Ray Report', 'medical_report', 'ipfs_hash_456', now() - interval '22 days', now() - interval '20 days'),
        ('00000000-0000-0000-0000-000000000203'::uuid, patient1_id, 'Vaccination Record', 'medical_report', 'ipfs_hash_789', now() - interval '30 days', now() - interval '30 days'),
        ('00000000-0000-0000-0000-000000000204'::uuid, patient2_id, 'MRI Scan', 'medical_report', 'ipfs_hash_abc', now() - interval '18 days', now() - interval '15 days'),
        ('00000000-0000-0000-0000-000000000205'::uuid, patient3_id, 'Allergy Test Results', 'medical_report', 'ipfs_hash_def', now() - interval '28 days', now() - interval '25 days'),
        ('00000000-0000-0000-0000-000000000206'::uuid, patient2_id, 'Annual Physical Results', 'medical_report', 'ipfs_hash_ghi', now() - interval '42 days', now() - interval '40 days');
